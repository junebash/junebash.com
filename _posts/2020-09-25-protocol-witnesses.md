...
I've been going through [PointFree](http://pointfree.co)'s video backlog lately and have been learning a ton, but their concept of using a generic struct as a replacement for the limitations of protocols has been somewhat relavatory for me. If you've ever been frustrated like I have working with protocols and running into issues with `associatedType`s and `Self`s and type erasure, then you may have your mind blown a little bit. I'd like to briefly show how this works using an example after showing a couple of alternatives. <!--more-->

## The Scenario

Let's say we're working on a networking class for a social media app. Our backend uses GraphQL, but we using the Apollo framework would be overkill for our purposes, so we're handling the network code ourselves. (We'll also be using the Combine framework's publishers rather than traditional completion closures; I'll be writing a separate post about that in the near future.)
...
How will the `query` method know what the output is, though? Maybe we can add a computed property to the enum.
...
...but when we go to adjust our query method, we run into some problems...
...
However, there's a few problems. Here's what it looks like when we call this method for fetching friends:
...
That dreaded Self/associated type error. This is, as far as I've been able to tell, a limitation of the language, and it's why complicated type erasure exists. I'll be writing a separate post later about how to accomplish this kind of type erasure, but there's a much simpler, more elegant, and, in my opinion, more interesting solution.
...
extension QueryInput where Output == UUID {
    static func addPerson(_ person: Person) -> QueryInput {
...
We can replace `QueryInput<UUID>` with `Self`, since the `where` clause in the extension signature already tells us that that is the type we're working on. This will make it easier as we add more static convenience methods for our different queries.
...
The first has an additional layer of nesting, which the second avoids. Which you prefer might depend on the context; you could even keep both if it made sense!
...
extension QueryInput where Output == Person {
    static func fetchPerson(withName name: String) -> QueryInput {
        Self(name, queryString: "<insert query string here>")
    }

    static func fetchOrganizer(forEventID id: UUID) -> QueryInput {
